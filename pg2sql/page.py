...
# 同一数据目录的页面布局一致；缓存校验失败自动回退三阶段探测，安全。
_LAYOUT_CACHE = {"psv_offset": None, "version": None, "header_size": None}


def _update_layout_cache(psv_offset, version, header_size):
...
class Page:
    ...
    def __init__(self, pageno: int, raw: bytes):
        self.pageno = pageno
        self.raw = raw
        self.header = {}
        self.items = []
    ...
    def _parse(self):
        if len(self.raw) != PAGE_SIZE:
            self.error = f"page size {len(self.raw)} != {PAGE_SIZE}"
            return
    ...
    def _try_cached_layout(self) -> bool:
        ...
        if (psv & 0x7F) != version or (psv & ~0x7F) != PAGE_SIZE:
            return False
        ...
        if not (header_end <= pd_lower <= pd_upper <= pd_special <= PAGE_SIZE):
            return False
    ...
    def _try_standard_layout(self) -> bool:
        ...
        version = psv & 0x7F
        size = psv & ~0x7F
        if version != PG_PAGE_VERSION or size != PAGE_SIZE:
            return False
        ...
        if not (24 <= pd_lower <= pd_upper <= pd_special <= PAGE_SIZE):
            return False
        ...
        self._parse_items(pd_lower)
        self.has_valid_layout = True
        _update_layout_cache(18, version, 24)  # 记住布局供后续页快速解析
        return True
    ...
    def _try_auto_detect_layout(self) -> bool:
        ...
        for off in range(8, 64, 2):
            ...
            # 接受 version 4 或 5, size 必须等于 PAGE_SIZE
            ...
            self._parse_items(pd_lower)
            self.has_valid_layout = True
            _update_layout_cache(off, version, header_end)
            return True
    ...
    def _try_loose_detect_layout(self) -> bool:
        ...
        接受 version 4-5 和 size == 8192 的组合。
        ...
        for off in range(8, 128, 2):
            ...
            # 接受 version 4-5, size 必须等于 PAGE_SIZE
            if version not in (PG_PAGE_VERSION, KB_PAGE_VERSION):
                continue
            if size != PAGE_SIZE:
                continue
    ...
    def _parse_items(self, pd_lower: int):
        ...
        for i in range(n_items):
            ...
            if it.flags == ITEMID_UNUSED:
                continue
            # 校验 off 和 len 在合理范围内
            if it.off >= PAGE_SIZE or it.len > PAGE_SIZE:
                continue
            if it.off + it.len > PAGE_SIZE:
                continue
            items.append(it)
        self.items = items
